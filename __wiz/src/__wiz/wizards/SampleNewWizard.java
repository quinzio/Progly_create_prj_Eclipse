...
public class SampleNewWizard extends Wizard implements INewWizard {
	...
	private void doFinish(
		...
		IProject newProjectHandle = wrkSpaceRoot.getProject("prjAAA");
		try {
			monitor.beginTask("creating CDT Project ", 0);
			IProjectDescription projDesc = ResourcesPlugin.getWorkspace()
					.newProjectDescription(newProjectHandle.getName());
			projDesc.s
			if (!("".equals(containerName)) && containerName != null) {
				Path myPath = new Path(containerName);
	...
	private void throwCoreException(String message) throws CoreException {
		IStatus status =
			new Status(IStatus.ERROR, "__wiz", IStatus.OK, message, null);
